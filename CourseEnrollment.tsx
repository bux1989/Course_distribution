...
import { AppDocumentationDialog } from "./AppDocumentationDialog";
import { WeWebStepByStepGuide } from "./WeWebStepByStepGuide";

export function CourseEnrollment() {
  const [semesters, setSemesters] =
    useState<Semester[]>(mockSemesters);
  const [selectedSemesterIndex, setSelectedSemesterIndex] =
    useState<number>(0);
  const [selectedDayIndex, setSelectedDayIndex] =
    useState<number>(0);
  const [searchQuery, setSearchQuery] = useState<string>("");

  const [activePopoverForm, setActivePopoverForm] = useState<{
    childId: string;
    courseId: string;
    dayIndex: number;
  } | null>(null);
  ...
  const [courseDetailsVisible, setCourseDetailsVisible] =
    useState<{ [courseKey: string]: boolean }>({});
  ...
  const autoEnrollStudents = () => {
    const updatedSemester = processAutomaticEnrollment(
              ...
              courses: day.courses.map((course) => {
                ...
                if (course.enrolledChildren.includes(childId)) {
                  return {
                    ...course,
                    enrolledChildren:
                      course.enrolledChildren.filter(
                        (id) => id !== childId,
                      ),
                  };
                }
  ...
  const getUnenrolledChildren = () => {
    const enrolledIds = currentDay.courses.flatMap(
      (course) => course.enrolledChildren,
    );
    return currentSemester.children.filter(
      (child) =>
        ...
        !(
          child.enrollments[selectedDayIndex] === undefined &&
                  ...
                  ? {
                      ...
                      notes: course.notes.map((note) =>
                        note.id === noteId
                          ? {
                              ...note,
                              isProblem: !note.isProblem,
                              isResolved: note.isProblem
                                ? false
                                : note.isResolved,
                            }
                          : note,
  ...
  const renderSchedulePopover = (
    ...
    const preferences = getChildPreferences(
      childId,
      currentSemester,
      currentDay,
    );
    const currentChoice = preferences.find(
      (p) => p.courseId === activity,
    );
    const enrolledChildren =
      activity !== "go-home"
        ...
        : [];
    const isPopoverFormOpen =
      activePopoverForm?.childId === childId &&
      activePopoverForm?.dayIndex === dayIndex;

    const renderPopoverInlineForm = (
      targetCourseId: string,
    ) => {
      const availableSpots = getCourseAvailableSpotsForDay(
        dayIndex,
        targetCourseId,
        currentSemester,
      );
      ...
      const targetEnrolledChildren = isGoHome
        ? []
        : getChildrenInCourseForDay(
            dayIndex,
            targetCourseId,
      ...
      return (
        <div className="mt-2 p-2 bg-accent/30 rounded border text-xs">
          ...
          {!isGoHome && targetEnrolledChildren.length > 0 && (
            <div className="mb-2">
              ...
              <div className="space-y-1">
                {sortChildrenByClassAndName(
                  targetEnrolledChildren,
                ).map((child) => {
                  const availableMoveCourses =
                    getAvailableCoursesForMoveForDay(
                      dayIndex,
                      targetCourseId,
                      currentSemester,
                  ...
                  return (
                    <div
                      key={child?.id}
                      className="flex items-center justify-between py-0.5 px-1 bg-background/80 rounded"
                    >
                      <div className="flex items-center gap-1">
          ...
          <Button
            ...
            onClick={() => {
              const blockingReason = getMovementBlockingReason(
                childId,
                targetCourseId,
                currentSemester,
                dayIndex,
      ...
      <PopoverContent className="w-64 p-2">
        <div className="space-y-2">
          <p className="text-xs mb-2">
            {day} -{" "}
            {activity === "go-home" ? "Going Home" : activity}
          </p>

          {currentChoice && (
            <div className="mb-2 p-1 bg-accent/50 rounded">
              <p className="text-xs">
                Current: {currentChoice.choice} choice
              </p>
            </div>
          )}
                ...
                >
                  ...
                  {courseId !== "go-home" && (
                    <span className="ml-1 text-muted-foreground">
                      {(() => {
                        const day =
                          currentSemester.schedule[dayIndex];
                        const course = day.courses.find(
                          (c) => c.id === courseId,
                        );
                        return course
          ...
          {isPopoverFormOpen &&
            activePopoverForm &&
            renderPopoverInlineForm(activePopoverForm.courseId)}

          {activity !== "go-home" && (
            <div className="space-y-1 border-t pt-2">
              <p className="text-xs text-muted-foreground">
                Other students enrolled:{" "}
                {getCourseName(activity, currentDay)}
                {currentChoice &&
              ...
              {enrolledChildren.length > 0 ? (
                <div className="space-y-1">
                  {sortChildrenByClassAndName(
                    ...
                    >
                      <span>
                        {courseChild?.class} {courseChild?.name}
                      </span>
                    </div>
                  ))}
                </div>
        ...
        <div className="space-y-2">
          ...
          {!isGoHome && enrolledChildren.length > 0 && (
            <div className="border-t pt-2">
              <p className="text-xs text-muted-foreground mb-1">
                Currently enrolled ({enrolledChildren.length}):
              </p>
  ...
  const getCourseAvailableSpotsForDay = (
    dayIndex: number,
    courseId: string,
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex items-center gap-4 mb-4">
        ...
        <WeWebStepByStepGuide />

        <AppDocumentationDialog />
        ...
        <Select
          value={selectedSemesterIndex.toString()}
          onValueChange={(value) =>
            setSelectedSemesterIndex(parseInt(value))
          }
        >
        ...
        <div className="mb-4 p-3 bg-muted/30 rounded border-l-2 border-muted-foreground">
          ...
          <div className="space-y-1">
            {sortChildrenByClassAndName(
              ...
              const schedule = getChildSchedule(
                child.id,
                currentSemester,
                true,
                selectedDayIndex,
      ...
      <div className="space-y-3">
        {currentDay.courses.map((course) => {
          const enrolledChildren = filteredChildren(
            getChildrenInCourse(
              course.id,
              currentDay,
              currentSemester,
          ...
          const isFull =
            course.enrolledChildren.length >=
              course.maxCapacity || course.forcedFull;
          const isOverfilled =
            course.enrolledChildren.length > course.maxCapacity;
          ...
          return (
            <div
              key={course.id}
              className={`rounded p-3 ${isOverfilled ? "bg-red-50 border border-red-200" : "bg-muted/20"}`}
            >
              <div className="flex items-center justify-between mb-2">
                <h4 className="flex items-center gap-2">
                  <span
                    ...
                    onClick={() =>
                      toggleCourseDetailsVisibility(course.id)
                    }
                  >
                    {course.name}
                  </span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      // Ensure course details and notes are visible
                      setCourseDetailsVisible((prev) => ({
                        ...prev,
                        [courseKey]: true,
                      }));
              ...
              {isCourseDetailsVisible(course.id) && (
                <div className="mb-2 text-xs text-muted-foreground flex items-center gap-4">
                  ...
                  <span>
                    Grades: {course.availableGrades.join(", ")}
                  </span>
                  {course.notes.length > 0 && (
                    <div className="flex items-center gap-1">
                      ...
                      <Button
                        ...
                        className="h-4 w-4 p-0"
                        onClick={() =>
                          toggleNotesVisibility(course.id)
                        }
                        title={
                              ...
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                  ...
                                  {note.isResolved && (
                                    <Badge
                                      variant="outline"
                                      className="text-xs px-1 py-0 bg-green-100 text-green-700 border-green-300"
                                    >
                                      Resolved
                                    </Badge>
                                  )}
                  ...
                  <div className="flex items-center justify-between py-1 px-2 bg-muted/50 rounded text-xs font-medium text-muted-foreground border-b border-border">
                    ...
                    <div className="ml-2">
                      <Button
                        ...
                        className="h-4 w-4 p-0"
                        onClick={() =>
                          toggleChildrenVisibility(course.id)
                        }
                        title={
                          isChildrenVisible(course.id)
                            ? "Hide participants"
                    ...
                    <>
                      {sortChildrenByClassAndName(
                        ...
                        return (
                          <div
                            key={child.id}
                            className="flex items-center justify-between py-1 px-2 bg-background rounded text-sm"
                          >
                            <div className="grid grid-cols-12 gap-2 flex-1">
                              {/* Participants Column */}
